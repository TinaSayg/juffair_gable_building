...
<section class="section">
    ...
    <div class="col-12" >
        <div class="card">
            ...
            <div class="card-body">
            <form method="POSt" action="{{ isset($task) ? route('tasks.update', $task->id) : '' }}" enctype="multipart/form-data">
              @csrf
                <div class="row">
                    ...
                    <div class="form-group col-md-4">
                        <label>Date</label>
                        <input type="text" name="assign_date" value="{{ isset($task) ? $task->assign_date : '' }}" class="form-control datepicker">
                    </div>
                    <div class="form-group col-md-4">
                        <label>Time</label>
                        <div class="input-group">
                          ...
                          <input type="text"  name="assign_time" value="{{ isset($task) ? $task->assign_time : '' }}" class="form-control timepicker">
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control" name="task_status_code" >
                            @foreach ($task_status_list as $item)
                                <option value="{{ $item->task_status_code }}" {{(isset($task) && ($item->task_status_code == $task->task_status_code)) ? 'selected' : '' }}>{{ $item->task_status_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control" name="assignee_id" >
                            ...
                            @foreach ($employee_list as $item)
                                <option value="{{ $item->id }}" {{(isset($task) && ($item->id == $task->assignee_id)) ? 'selected' : '' }}>{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
