        ...
        <div class="col-12">
            <div class="col-12">
                <div class="card" style="padding:15px 15px">
                    <form action="{{ route('units.apartment_by_color.list') }}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="form-group col-md-6" style="margin-top: 1.90rem !important;">
                                <label for="">Apartment Color</label>
                                <select name="apartment_color" class="form-control" id="">
                                    <option value="0">--- Select ---</option>
                                    @foreach ($apartment_colors as $item)
                                        <option value="{{ $item}}" style="background-color: {{$item}}; padding:5px;">{{ $item }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class="table-responsive">
                    ...
                    <tbody>
                        ...
                        @foreach($units as $unit)
                            ...
                            <tr @if($user_type == 'officer') style="cursor: pointer" class='clickable-row' data-href='{{route('units.apartment_by_color.show', $unit->id)}}' @endif>
